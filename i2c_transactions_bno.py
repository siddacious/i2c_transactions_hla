...
#     my_choices_setting = ChoicesSetting(choices=('A', 'B'))
    ...
    result_types = {
        ...
        'bno_header': {
            'format': '{{data.channel}} {{data.data_len}} {{data.sequence_number}} {{data.packet_data}}'
        },
        'bno_report': {
            'format': '{{data.data}}'
        },
    ...
    def process_transaction(self):
        ...
        #     self.current_transaction.start_time, self.current_transaction.end_time, {
        #     'input_type': self.current_frame.type, 'transaction_string':transaction_string, 'test':'foobar'
        # })  
        bno_decoded = self.decoder.decode_transaction(self.current_transaction)
        print("bno_decoded:", bno_decoded)
        if bno_decoded:
            new_frame = AnalyzerFrame(
                ...
                {
                    "channel" : bno_decoded['header']['channel'], 
                    "data_len": bno_decoded['header']['data_len'], 
                    "sequence_number": bno_decoded['header']['sequence_number'],
                    "packet_data": bno_decoded['data'],
                    "read" : not bno_decoded['write']
                }
            )
            return new_frame
