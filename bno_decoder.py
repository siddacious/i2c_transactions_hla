...
class BNODecoder:
    ...
    def get_sensor_report(self, data):

        i_raw = unpack_from("<h", data, offset=9)[0]
        j_raw = unpack_from("<h", data, offset=11)[0]
        k_raw = unpack_from("<h", data, offset=13)[0]   

        return (i_raw, j_raw, k_raw)     
        ...
        # FB 2A 28 00 00  05  E1  00  00   9A 00  F4 00  AA 34  59 24  44 32  05 E2 04 F8 9A 00 F4 00 AA

        # 34 59 24 44 32  05  E3  0C  EC   9A 00  F4 00  AA 34  59 24  44 32  05 E4 14 E0 99 00 F5 00 AA
        # 34 59 24 44 32  05  E5  1C  D6   99 00  F4 00  AA 34  59 24  44 32  05 E6 24 CA 99 00 F5 00 AA
        # 34 59 24 44 32  05  E7  2C  BE   99 00  F5 00  AA 34  59 24  44 32  05 E8 34 B3 98 00 F4 00 AA
        
        # 34 5A 24 44 32  05  E9  3C  A8   97 00  F4 00  AA 34  59 24  44 32  05 EA 44 9C 97 00 F4 00 AA
        # 34 5A 24 44 32  05  EB  4C  90   97 00  F4 00  AA 34  5A 24  44 32  05 EC 54 86 98 00 F4 00 A9



    def decode_transaction(self, reg_txn):
        packet_data = reg_txn.data
        CHANNELS = [
            "Command",
            "Exe",
            "Control",
        ...
        print("Before stuff: len(packet_data)", len(packet_data))

        if len(packet_data) <= 4:
            return None
        data_start=4
        body_data = packet_data[data_start:]
        packet_byte_count, channel_number, sequence_number = self.get_header(packet_data)
       
        if channel_number <=5:
            channel_str = CHANNELS[channel_number]
        else:
            channel_str = str(channel_number)
        header = {
            "data_len": packet_byte_count-4,
            "channel": channel_str,
            "sequence_number": sequence_number
        }
        
        if channel_number == 3 and len(packet_data) > 15 and  packet_data[5] == 5:
            print("IJKin")
            i, j, k = self.get_sensor_report(packet_data)
        print("")
        return {"header": header, "data": body_data,
            "write" : reg_txn.write}
